...
from gptui.views.tui import MainApp
...
def gptui():
    ...
    gptui_run(config_path=config_path, config=config)
   
def gptui_run(config_path: str, config: dict) -> None:
...
if __name__ == "__main__":
    # Retrieve config from config path.
    try:
        with open(".default_config.yml", "r") as default_config_file:
            config = yaml.safe_load(default_config_file)
    except FileNotFoundError:
        print(f"Default config file '.default_config.yml' is not found.")
        sys.exit(1)
    ...
    gptui_run(config_path='./config.yml', config=config)
